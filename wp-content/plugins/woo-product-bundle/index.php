<?php
/*
Plugin Name: WooCommerce Product Bundle
	...
	class WooSB {
		...
		function woosb_settings_link( $links, $file ) {
			...
			if ( $plugin == $file ) {
				...
				$links[]       = '<a href="' . WOOSB_PRO_URL . '">' . esc_html__( 'Premium Version', 'woosb' ) . '</a>';
		...
		function woosb_get_search_results() {
			...
			if ( is_array( $ids_arrs ) && count( $ids_arrs ) > 2 ) {
				echo '<ul><span>Please use the Premium Version to add more than 3 products into the bundle & get lifetime support. Click <a href="' . WOOSB_PRO_URL . '" target="_blank">here</a> to buy, just $19!</span></ul>';
			} else {
